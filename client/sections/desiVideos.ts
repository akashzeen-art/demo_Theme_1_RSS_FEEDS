...
export function findVideoEntry(title: string): DesiVideoEntry | undefined {
  ...
  const exact = LATEST_VIDEOS.find((v) => normalizeTitle(v.title) === n);
  if (exact) return exact;
  return LATEST_VIDEOS.find(
    (v) =>
      n.includes(normalizeTitle(v.title)) ||
      normalizeTitle(v.title).includes(n)
  );
}

/** Title → duration / rating from catalog (falls back safely). */
export function getVideoMetaByTitle(title: string): Pick<DesiVideoEntry, 'duration' | 'rating' | 'title' | 'url' | 'thumb'> {
  const entry = findVideoEntry(title);
  if (entry) return entry;
  return {
    title,
    url: LATEST_VIDEOS[0].url,
    thumb: LATEST_VIDEOS[0].thumb,
    duration: '30 min',
    rating: '4.5',
  };
}
